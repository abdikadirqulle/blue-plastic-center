...
import type {
  LedgerRepository,
  TrialBalanceRow,
} from "../accounting/ledger-repository.js";
...
function agingBucket(dueDate: unknown, asOf: string) {
  const days = Math.floor(
    (new Date(asOf).getTime() - new Date(String(dueDate)).getTime()) /
      86_400_000,
  );
...
function filterRows(kind: string, rows: TrialBalanceRow[]) {
  if (kind === "profit-and-loss") {
    return rows.filter((row) =>
      ["income", "expense", "cost-of-goods-sold"].includes(
        row.accountType ?? "",
      ),
    );
  }
  ...
  if (kind === "cash-flow") {
    return rows.filter(
      (row) =>
        ...
        /cash|bank/i.test(row.accountName ?? row.accountId),
    );
  }
  return rows;
}
    ...
    async (request, reply) => {
        ...
        return reply
          ...
          .send({
            error: {
              ...
              message: "Unknown financial report",
            },
          });
      }
      ...
      if (
        ...
        const [invoices, items, customers, payments] = await Promise.all([
          ...
          resources.list(request.requestContext, "inventory", "items", {
            page: 1,
            pageSize: 100,
            order: "desc",
          }),
          resources.list(request.requestContext, "sales", "customers", {
            page: 1,
            pageSize: 100,
            order: "desc",
          }),
          resources.list(request.requestContext, "sales", "payments", {
            page: 1,
            pageSize: 100,
            order: "desc",
          }),
        ...
        const itemNames = new Map(
          items.data.map((record) => [
            record.id,
            String(record.data.name ?? record.data.displayName ?? "Item"),
          ]),
        );
        ...
        const invoiceNumbers = new Map(
          invoices.data.map((record) => [
            record.id,
            String(record.data.documentNumber ?? "Invoice"),
          ]),
        );
        ...
        if (request.params.kind === "sales-by-item") {
          const totals = new Map<
            string,
            { item: string; quantity: number; sales: number }
          >();
          for (const invoice of invoices.data) {
            if (
              String(invoice.data.invoiceDate ?? "").slice(0, 10) <
                parameters.from ||
              String(invoice.data.invoiceDate ?? "").slice(0, 10) >
                parameters.to
            )
              continue;
            for (const line of Array.isArray(invoice.data.lines)
              ? (invoice.data.lines as Array<Record<string, unknown>>)
              : []) {
              const item =
                itemNames.get(String(line.itemId)) ??
                String(line.description ?? "Unspecified item");
              const current = totals.get(item) ?? {
                item,
                quantity: 0,
                sales: 0,
              };
          ...
          rows = [...totals.values()].map((row) => ({
            item: row.item,
            quantitySold: row.quantity.toFixed(2),
          ...
          for (const invoice of invoices.data) {
            if (
              ...
              continue;
            const customer =
              customerNames.get(String(invoice.data.customerId)) ??
              String(invoice.data.customerName ?? "Unspecified customer");
            const current = totals.get(customer) ?? {
              customer,
              invoices: 0,
              sales: 0,
              balance: 0,
          ...
          rows = [...totals.values()].map((row) => ({
            ...
            totalSales: row.sales.toFixed(2),
            balanceDue: row.balance.toFixed(2),
          }));
          ...
          rows = payments.data
            .filter(
              (payment) =>
                String(payment.data.paymentDate ?? "").slice(0, 10) >=
                  parameters.from &&
                String(payment.data.paymentDate ?? "").slice(0, 10) <=
                  parameters.to,
            )
            .map((payment) => ({
              ...
              customer:
                customerNames.get(String(payment.data.customerId)) ??
                payment.data.customerName ??
                "Unspecified customer",
              date: payment.data.paymentDate,
              method: payment.data.paymentMethod,
              reference: payment.data.reference,
              amount: payment.data.amount,
              appliedTo: Array.isArray(payment.data.allocations)
                ? (payment.data.allocations as Array<Record<string, unknown>>)
                    .map(
                      (allocation) =>
                        invoiceNumbers.get(String(allocation.invoiceId)) ??
                        "Invoice",
                    )
                    .join(", ")
                : "Unapplied",
      ...
      if (source) {
        ...
        const [customers, vendors, invoices, bills, items] = await Promise.all([
          ...
          resources.list(request.requestContext, "purchasing", "vendors", {
            page: 1,
            pageSize: 100,
            order: "desc",
          }),
          ...
          resources.list(request.requestContext, "purchasing", "bills", {
            page: 1,
            pageSize: 100,
            order: "desc",
          }),
        ...
        const rows = records.data.map((record) => {
          if (request.params.kind.endsWith("-aging")) {
            const partyKey =
              request.params.kind === "receivables-aging"
                ? "customerId"
                : "vendorId";
            const documentKey =
              request.params.kind === "receivables-aging"
            ...
            return {
              [request.params.kind === "receivables-aging"
                ? "customer"
                : "vendor"]:
                names.get(String(record.data[partyKey])) ?? "Unknown",
              document:
                names.get(String(record.data[documentKey])) ??
                record.data.documentNumber ??
                "—",
          ...
          if (request.params.kind === "inventory-valuation") {
            ...
            return {
              id: record.id,
              ...record.data,
              inventoryValue: minorToDecimal(value),
            };
          }
      ...
      const reportFrom =
        request.params.kind === "balance-sheet"
          ? "2000-01-01"
          : parameters.from;
