...
export function LoadingState({
  ...
  if (compact)
    return (
      <span
        className={cn(
          "inline-flex items-center gap-2 text-xs text-[#607681]",
          className,
        )}
      >
        <LoaderCircle size={14} className="animate-spin text-[#007DCC]" />
        {label}
      </span>
    )
  ...
  return (
    <div
      className={cn(
        "space-y-3 rounded-xl border border-[#e3eaf0] bg-white p-5",
        className,
      )}
      role="status"
      aria-live="polite"
