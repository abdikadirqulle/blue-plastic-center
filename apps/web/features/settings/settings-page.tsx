...
export function SettingsPage({ activeSection }: { activeSection: string }) {
  ...
  const users = useQuery({
    queryKey: ["auth", "users"],
    queryFn: () => apiClient.getPath<Array<{ id: string; username: string; email: string; displayName: string; role: string; active: boolean }>>("/v1/auth/users"),
    enabled: isUsers,
  })
...
function UsersManager({ query, onToast }: { query: ReturnType<typeof useQuery<{ data: UserRow[] }>>; onToast: (toast: ToastMessage) => void }) {
  ...
  const openUser = (user?: UserRow) => { setEditing(user ?? null); setDisplayName(user?.displayName ?? ""); setUsername(user?.username ?? ""); setEmail(user?.email ?? ""); setRole(user?.role ?? "viewer"); setPassword(""); setModal("user") }
  const refresh = () => client.invalidateQueries({ queryKey: ["auth", "users"] })
  const saveUser = useMutation({
    mutationFn: () => editing
      ? apiClient.action<UserRow>(`/v1/auth/users/${editing.id}`, { displayName, username, email, role }, "PATCH")
      : apiClient.action<UserRow>("/v1/auth/users", { displayName, username, email, role, password }),
