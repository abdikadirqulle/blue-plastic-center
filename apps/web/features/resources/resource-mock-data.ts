...
const valuesByColumn: Record<string, string[]> = {
  ...
  "Legal name": [
    "BLUE PLASTIC CENTER LLC",
    "Blue Plastic Trading Ltd.",
    "Blue Plastic Logistics",
    "Blue Plastic Retail",
    "Blue Plastic Services",
  ],
...
function contextualValue(
  config: ResourceConfig,
  column: string,
  index: number,
) {
  if (/date$/i.test(column) || ["Due date", "Next date"].includes(column))
    return dates[index]
  if (config.module === "debts" && column === "Type")
    return [
      "Customer receipt",
      "Vendor payment",
      "Customer receipt",
  ...
  if (column === "Status")
    return (
      statusesByResource[`${config.module}/${config.slug}`]?.[index] ??
      statusesByResource[config.slug]?.[index] ??
      "Active"
    )
  if (column === "PO") return `PO-${String(780 + index)}`
  if (column === "Invoice") return `INV-${String(1048 - index)}`
  if (column === "SKU")
    return ["CEM-50", "RICE-25", "OIL-20", "STL-12", "CHR-PRO"][index]
...
export function createResourceRows(config: ResourceConfig): ResourceRow[] {
  ...
  const statuses = statusesByResource[`${config.module}/${config.slug}`] ??
    statusesByResource[config.slug] ?? [
      "Active",
      "Pending",
      "Completed",
      "Draft",
  ...
  return Array.from({ length: 5 }, (_, index) => ({
    ...
    cells: config.columns
      .slice(1)
      .map((column) => contextualValue(config, column, index)),
    status: statuses[index],
  }))
}

export function getResourcePresentation(
  module: string,
  slug: string,
): ResourceConfig["presentation"] {
  if (
    [
      "cash-flow",
      "reconciliation",
