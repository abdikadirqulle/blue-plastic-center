...
function displayValue(field: FormField, index: number, row: ResourceRow) {
  ...
  if (/amount|price|balance|total|budget|value|cost|principal|rate/i.test(field.name)) return row.cells[1] ?? "$0.00";
  ...
  if (/memo|note|description/i.test(field.name)) return `Recorded for ${row.cells[0] ?? row.id}. Verified against the original transaction and supporting documents.`;
    ...
    <AppShell>
      <div className="mx-auto max-w-[1500px]">
        <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
          <div>
            <Link href={listHref} className="mb-3 inline-flex items-center gap-2 text-xs font-bold text-[#007DCC]"><ArrowLeft size={15}/> Back to {config.title.toLowerCase()}</Link>
            <div className="flex flex-wrap items-center gap-3">
              <h1 className="text-2xl font-bold tracking-[-0.035em] text-[#142735] md:text-[29px]">{row.id}</h1>
              <Badge variant={statusVariant(row.status)}>{row.status}</Badge>
            </div>
            <p className="mt-1.5 text-sm font-semibold text-[#526a76]">{row.cells[0]}</p>
          </div>
          ...
          <div className="space-y-4">
            <Card className="grid gap-4 p-5 sm:grid-cols-2 lg:grid-cols-4">
              {config.columns.slice(0, 4).map((column, index) => (
                <div key={column}>
                  <p className="text-[10px] font-bold uppercase tracking-wide text-[#81929c]">{column}</p>
                  <p className="mt-1.5 text-sm font-bold text-[#29424e]">{index === 0 ? row.id : row.cells[index - 1] ?? "—"}</p>
                </div>
              ))}
            </Card>
            ...
            {config.formSections.map((section) => (
              <Card key={section.title} className="overflow-hidden">
                <div className="border-b border-[#e8eef2] px-5 py-4"><h2 className="text-sm font-bold text-[#263f4b]">{section.title}</h2>{section.description ? <p className="mt-1 text-xs text-[#7b8d97]">{section.description}</p> : null}</div>
                <dl className="grid md:grid-cols-2">
                  {section.fields.map((field, index) => (
                    <div key={field.name} className="border-b border-[#edf1f4] px-5 py-4 md:odd:border-r">
                      <dt className="text-[10px] font-bold uppercase tracking-wide text-[#81929c]">{field.label}</dt>
                      <dd className="mt-1.5 text-sm font-semibold leading-6 text-[#304954]">{displayValue(field, index, row)}</dd>
                    </div>
                  ))}
                </dl>
              </Card>
            ...
            {config.hasLineItems ? (
              <Card className="overflow-hidden">
                <div className="border-b border-[#e8eef2] px-5 py-4"><h2 className="text-sm font-bold text-[#263f4b]">Transaction lines</h2></div>
                <div className="overflow-x-auto">
                  <table className="w-full min-w-[720px] text-left text-xs">
                    <thead className="bg-[#f8fafc] text-[10px] uppercase text-[#7b8e98]"><tr>{["Item / account","Description","Qty","Rate","Tax","Amount"].map((item)=><th key={item} className="px-5 py-3">{item}</th>)}</tr></thead>
                    <tbody><tr className="border-t border-[#edf1f4]"><td className="px-5 py-4 font-bold">{row.cells[0]}</td><td className="px-5 py-4">Primary transaction line</td><td className="px-5 py-4">1</td><td className="px-5 py-4">{row.cells[1]}</td><td className="px-5 py-4">Standard</td><td className="px-5 py-4 font-bold">{row.cells[1]}</td></tr></tbody>
                  </table>
                </div>
              </Card>
            ) : null}
          ...
          <div className="space-y-4">
            ...
            <Card className="p-5">
              ...
              <div className="mt-4 space-y-4">
                {["Record created","Details verified","Last updated"].map((item,index)=><div key={item} className="flex gap-3"><CheckCircle2 size={16} className="mt-0.5 text-emerald-500"/><div><p className="text-xs font-bold text-[#405762]">{item}</p><p className="mt-1 text-[10px] text-[#82949e]">{index === 0 ? "25 Jul 2026 · Abdikadir" : "26 Jul 2026 · Finance team"}</p></div></div>)}
              </div>
            </Card>
          </div>
        </div>
