...
test("server-renders the BLUE PLASTIC CENTER dashboard", async () => {
  ...
  assert.match(html, /Good evening, Abdisalam/)
...
test("keeps the frontend foundation documented and modular", async () => {
  const [
    ...
    readFile(
      new URL("../features/resources/resource-page.tsx", import.meta.url),
      "utf8",
    ),
    readFile(
      new URL("../features/resources/resource-form-page.tsx", import.meta.url),
      "utf8",
    ),
    readFile(
...
test("renders the dashboard period control and complete header menus", async () => {
  ...
  assert.match(shell, /Open search/)
  assert.match(shell, /createGroups/)
  assert.match(shell, /People & projects/)
  assert.match(shell, /rounded-full/)
  assert.doesNotMatch(shell, /Switch company/)
  assert.doesNotMatch(navigation, /Main company/)
...
test("uses the shared delete confirmation dialog for every destructive record action", async () => {
  const [dialog, resourcePage, detailsPage, formPage] = await Promise.all([
    readFile(
      new URL("../components/ui/confirm-delete-dialog.tsx", import.meta.url),
      "utf8",
    ),
    readFile(
      new URL("../features/resources/resource-page.tsx", import.meta.url),
    ...
    readFile(
      new URL(
        "../features/resources/resource-details-page.tsx",
        import.meta.url,
      ),
      "utf8",
    ),
